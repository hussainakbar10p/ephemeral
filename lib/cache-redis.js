...
function setValueNX(cache_client, key, value, expiry_in_seconds, callback) {
    cache_client.setnx(key, value, function (err, response) {
        if (err) {
            callback(err);
            return;
        }
...
function subscribeInner(cache_client, eventName, callback) {
  cache_client.subscribe(eventName);
  cache_client.on("message", function(channel, message) {
...
RedisCache.prototype.setNX = function set(key, value, expiry_in_seconds) {
    var self = this;
    return getCacheClient(self).then(function(cache) {

        return Promise.promisify(setValueNX)(cache, key, value, expiry_in_seconds);
    });
};

RedisCache.prototype.subscribe = function subscribeMethod(eventName, callback) {
  var self = this;
  return getCacheClient(self).then(function(cache) {
    return Promise.promisify(subscribeInner)(cache, eventName, callback);
  });
};
